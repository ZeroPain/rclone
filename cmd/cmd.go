...
var (
	// Flags
	cpuProfile    = config.StringP("cpuprofile", "", "", "Write cpu profile to file")
	memProfile    = config.StringP("memprofile", "", "", "Write memory profile to file")
	statsInterval = config.DurationP("stats", "", time.Minute*1, "Interval between printing stats, e.g 500ms, 60s, 5m. (0 to disable)")
	dataRateUnit  = config.StringP("stats-unit", "", "bytes", "Show data rate in stats as either 'bits' or 'bytes'/s")
	version       bool
	retries       = config.IntP("retries", "", 3, "Retry operations this many times if they fail")
	// Errors
...
func init() {
	filterflags.AddFlags(Root.Flags())
}
