...
var (
	...
	userAgent             = StringP("user-agent", "", "rclone/"+fs.Version, "Set the user-agent to a specified string. The default is rclone/ version")
	...
	streamingUploadCutoff = fs.SizeSuffix(100 * 1024)
	dump                  fs.DumpFlags
	logLevel              = fs.LogLevelNotice
	statsLogLevel         = fs.LogLevelInfo
	bwLimit               fs.BwTimetable
	bufferSize            fs.SizeSuffix = 16 << 20
...
func init() {
	...
	VarP(&dump, "dump", "", "List of items to dump from: "+fs.DumpFlagsList)
...
func LoadConfig() {
	...
	if logLevelFlag != nil && logLevelFlag.Changed {
		...
		fs.Config.LogLevel = logLevel
	}
	fs.Config.StatsLogLevel = statsLogLevel
	fs.Config.ModifyWindow = *modifyWindow
	fs.Config.Checkers = *checkers
	fs.Config.Transfers = *transfers
	fs.Config.DryRun = *dryRun
	fs.Config.Timeout = *timeout
	fs.Config.ConnectTimeout = *connectTimeout
	fs.Config.CheckSum = *checkSum
	fs.Config.SizeOnly = *sizeOnly
	fs.Config.IgnoreTimes = *ignoreTimes
	fs.Config.IgnoreExisting = *ignoreExisting
	fs.Config.InsecureSkipVerify = *skipVerify
	fs.Config.LowLevelRetries = *lowLevelRetries
	fs.Config.UpdateOlder = *updateOlder
	fs.Config.NoGzip = *noGzip
	fs.Config.MaxDepth = *maxDepth
	fs.Config.IgnoreSize = *ignoreSize
	fs.Config.IgnoreChecksum = *ignoreChecksum
	fs.Config.NoTraverse = *noTraverse
	fs.Config.NoUpdateModTime = *noUpdateModTime
	fs.Config.BackupDir = *backupDir
	fs.Config.Suffix = *suffix
	fs.Config.UseListR = *useListR
	fs.Config.BwLimit = bwLimit
	fs.Config.TPSLimit = *tpsLimit
	fs.Config.TPSLimitBurst = *tpsLimitBurst
	fs.Config.UserAgent = *userAgent
	fs.Config.Immutable = *immutable
	fs.Config.AutoConfirm = *autoConfirm
	fs.Config.StatsFileNameLength = *statsFileNameLength
	fs.Config.BufferSize = bufferSize
	fs.Config.StreamingUploadCutoff = streamingUploadCutoff
	fs.Config.Dump = dump
	if *dumpHeaders {
	...
	fs.Config.TrackRenames = *trackRenames
